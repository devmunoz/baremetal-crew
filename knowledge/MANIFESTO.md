...
*   **Process:** The Dev and QA execute tasks sequentially. Every status update and bug event is logged via `bmc-log`:
    *   **Dev starts task:** Logs `.bmc-stuff/bin/bmc-log task SXX SXX-01 "In Development"`.
    *   **Dev completes task:** Updates documentation (`ARCHITECTURE.md`, `README.md`), writes unit tests, and logs `.bmc-stuff/bin/bmc-log task SXX SXX-01 "Ready for QA"`.
    *   **QA verifies task:** Runs E2E tests. If passed, logs `.bmc-stuff/bin/bmc-log task SXX SXX-01 "QA Passed"`. If failed, logs `.bmc-stuff/bin/bmc-log task SXX SXX-01 "Pending Correction" [RETRY-COUNT]`.
