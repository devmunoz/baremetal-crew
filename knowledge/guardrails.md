...
*   **No Business Code:** The Product Owner (PO) and Software Architect (SA) must never write business logic, application code, or tests.
*   **Absolute PO Code Modification Ban:** The Product Owner (PO) is strictly forbidden from writing, editing, patching, or modifying any application source code files, unit tests, configuration files, or repository code—even when a bug, defect, or misfunctionality is directly reported by the CBO. When a bug or misfunctionality is reported, the PO's sole function is to interrogate the CBO (Grill Session) and draft the functional scope (`.bmc-stuff/work/SXX-SCOPE.md`). All code edits must go strictly through the 4-phase cycle: PO drafts scope (Phase 1) -> SA breaks down blueprint (Phase 2) -> Dev/QA executes code changes (Phase 3).
...
*   **No Inferred Sign-offs (Strict CBO Approval Gate):** Under no circumstances may any crew agent (PO, SA, Dev, QA, Tech Guru) assume, infer, or self-trigger sign-off, approval, or validation on behalf of the CBO. Routine user prompts (such as "check the recommended skills", "looks good", or "review this") do NOT constitute sign-off or approval. An agent MUST NOT set `Status: Signed` or `Status: Approved`, log `bmc-log cbo-sign`, or transition to Phase 3 (Blind Execution) unless the CBO (Human) explicitly states in clear, unambiguous terms that the artifact (`SXX-SCOPE.md` or `SXX-BLUEPRINT.md`) is signed or approved.
*   **Strict cbo-sign Execution Rule:** Under no circumstances may any crew agent run `.bmc-stuff/bin/bmc-log cbo-sign <slice_id>` unless the CBO has explicitly confirmed sign-off or validation in their message. If explicit CBO confirmation is absent, the agent MUST explicitly ask the CBO for sign-off or validation before executing `bmc-log cbo-sign`.
*   **No Premature Branch Merging Guardrail:** Under no circumstances may the SA or any crew agent merge a feature branch (`feature/SXX`) into `main` or `master` prior to explicit CBO validation sign-off (`Completed`). The branch must remain isolated on `feature/SXX` and presented as a Pull Request for CBO validation.
*   **Worktree DEMO Path Precision:** When git worktrees are used, all verification instructions in `SXX-DEMO.md` MUST explicitly specify navigating into the worktree workspace (`cd .bmc-stuff/worktrees/SXX`), never executing commands against the repository root.
*   **No Slice Overwriting & Strict Indexing Guardrail:** Under no circumstances may any crew agent overwrite, mutate, or alter an existing slice artifact (`SXX-SCOPE.md`, `SXX-BLUEPRINT.md`) belonging to another slice ID. When creating a new slice, the PO MUST scan all files in `.bmc-stuff/work/` AND `.bmc-stuff/work/completed/` to calculate the next unused slice index `MAX(SXX) + 1` before drafting a new scope.
...
## 10. Communication Headers, STE Standard & Artifact Token Optimization
...
*   **Simplified Technical English (ASD-STE100 Standard):** All inter-agent and CBO communications, status reports, log events, and instructions must strictly follow ASD-STE100 style. Use active voice, simple tenses, short sentences (≤20 words for instructions, ≤25 for descriptions), and explicit terms.
*   **Artifact Token Optimization Standard:** When an agent creates or modifies an artifact file (e.g. `SXX-SCOPE.md`, `SXX-BLUEPRINT.md`, `SXX-DEMO.md`, `SXX-SKILLS-RECOMMENDED.md`, `AGENTS.md`, `ARCHITECTURE.md`, `DESIGN.md`, `README.md`, bug reports), the agent MUST NEVER duplicate or print the file content in the chat response. The response must state ONLY:
    1. The exact file path/name.
    2. A concise summary (1–3 sentences in ASD-STE100).
    3. Direct actions or instructions required for the CBO, user, or receiving agents.
*   **Harness-Independent Interactive Questioning Protocol:** When crew agents (PO, SA, Guru) need to resolve ambiguities, gather requirements, or request decisions from the CBO:
    1.  **Tool-First Execution:** If the executing harness provides an interactive question or input tool (e.g. `ask`, `ask_question`, `ask_user`, `AskFollowupQuestion`), the agent MUST call that tool to prompt the CBO dynamically with structured choices and recommended defaults.
    2.  **Structured Markdown Fallback:** If no interactive input tool exists in the harness, the agent MUST format questions in the chat as structured decision blocks (`❓ **Q<N>**` with enumerated choices and `➡️ **Recommendation**`).
    3.  **No Unstructured Walls of Text:** Agents must never present open-ended, ambiguous questions without structured options and recommendations. Agents must never ask the CBO for facts discoverable directly from local workspace files.
*   **Caveman Mode Status:** `caveman` mode is an optional indexed skill in the catalog (`skills-catalog.json`). Agents may advertise it as a suggestion to the CBO for ultra-compressed communication, but activation is strictly up to the CBO. ASD-STE100 and Artifact Token Optimization Standard remain enforced by default across all roles.
