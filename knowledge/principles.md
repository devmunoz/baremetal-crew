...
*   **Worktree Branch Isolation & No Premature Merging:** When git worktrees or `feature/SXX` branches are used, the feature branch MUST remain isolated until the CBO validates and signs off (`Completed`). Merging into `main` or `master` before explicit CBO sign-off is strictly prohibited.
...
*   **Framework Binary Protection:** Helper scripts under `bin/` or `.bmc-stuff/bin/` (`bmc-log`, `bmc-config`, `bmc-index-skills`) are immutable framework executables. Crew agents must NEVER edit, patch, or modify binary scripts.
...
*   **Interactive Questioning Protocol:** Crew agents must resolve ambiguities dynamically. Use native harness question tools (`ask`, `ask_question`, `ask_user`, `AskFollowupQuestion`) as the primary method with structured choices. Fall back to structured decision Markdown blocks when no interactive tool is available.
