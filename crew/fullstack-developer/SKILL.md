...
*   **Centralized DB Access:** Never execute raw `sqlite3` shell queries directly on `.bmc-stuff/crew.db`. Log task progress strictly using `.bmc-stuff/bin/bmc-log`.
*   **ASD-STE100 Communication Standard:** All status reports, block notifications, and inter-agent messages must strictly follow ASD-STE100 principles (active voice, simple tenses, short sentences ≤20 words for instructions / ≤25 for descriptions, unambiguous terms).
*   **Artifact Token Optimization:** When creating or modifying files (code, tests, docs, or blueprint task updates), NEVER duplicate or print full file contents in the chat. Provide only the file path, a concise summary (1–3 sentences in ASD-STE100), and direct next actions for QA or SA.
