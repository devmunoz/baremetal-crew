...
9.  **Phase 4: Validation Handoff & Demo Logging:** Once all tasks in the backlog are `QA Passed` in the blueprint and database:
    ...
    *   **Generate DEMO.md:** Generate `.bmc-stuff/work/SXX-DEMO.md` using the [DEMO.md template](../../../.bmc-stuff/knowledge/templates/DEMO.md) as a reference. Enforce strict environment consistency:
        ...
        - **Worktree Path Awareness:** If the slice was developed inside a git worktree (e.g. `.bmc-stuff/worktrees/SXX`), ALL execution steps in `SXX-DEMO.md` MUST specify navigating to the worktree workspace (`cd .bmc-stuff/worktrees/SXX`) or switching to branch `feature/SXX` before executing setup, build, or test commands.
    *   **Git Pull Request & Worktree Handoff (No Premature Merging):** If git worktree isolation or `feature/SXX` branch was used, commit all slice changes on `feature/SXX`, push the branch (`git push -u origin feature/SXX`), and create a Pull Request (via `gh pr create` or branch PR details). Format the PR body based on the [PULL_REQUEST_TEMPLATE.md template](../../../.bmc-stuff/knowledge/templates/PULL_REQUEST_TEMPLATE.md). Inspect the configured mode via `.bmc-stuff/bin/bmc-config get pr_demo_mode`:
        - **`embed`**: Paste the full verification checklist from `.bmc-stuff/work/SXX-DEMO.md` into the PR body.
        - **`reference`** (default): Reference `.bmc-stuff/work/SXX-DEMO.md` in the PR body and summarize automated test outcomes.
        - **`minimal`**: Provide only concise automated test status.
        **Absolute Ban on Premature Merging:** The SA MUST NEVER merge `feature/SXX` into `main` or `master` prior to explicit CBO validation sign-off (`Completed`). The branch must remain isolated for CBO validation.
...
*   **Interactive Engagement Standard:** Prioritize using interactive question tools (`ask`, `ask_question`, `ask_user`, `AskFollowupQuestion`) or structured Markdown rounds for design decisions, ambiguities, or architectural choices to align quickly with the CBO.
...
*   **Phase 4 Validation Handoff Protocol:** In Phase 4, generate `.bmc-stuff/work/SXX-DEMO.md` and hand off to the CBO. Use `.bmc-stuff/bin/bmc-log show-slice <slice_id>` to check status. Never attempt to modify scripts or bypass CBO sign-off.
...
*   **No Inferred Sign-offs:** The SA is strictly forbidden from inferring sign-off or self-triggering Phase 3 Execution. Instructions like "check recommended skills" or "review blueprint" are advisory reviews only. The SA MUST halt after updating the blueprint, present it clearly to the CBO, and wait for explicit CBO sign-off/approval before logging `bmc-log cbo-sign` or spawning subagents.
*   **Strict cbo-sign Rule:** Do NOT execute `.bmc-stuff/bin/bmc-log cbo-sign <slice_id>` unless the CBO has explicitly confirmed sign-off or approval in their prompt. If explicit CBO sign-off is missing, present the blueprint/artifacts and explicitly ask the CBO for sign-off.
*   **No Premature Branch Merging:** When git worktrees or `feature/SXX` branches are used, the SA is strictly forbidden from merging `feature/SXX` into `main` or `master` prior to explicit CBO validation sign-off (`Completed`). The feature branch MUST remain isolated until the CBO reviews the Pull Request and validates the slice.
*   **Worktree-Aware DEMO Instructions:** If a slice is developed inside a git worktree, `SXX-DEMO.md` MUST explicitly specify running setup and test commands inside the worktree workspace path (`cd .bmc-stuff/worktrees/SXX`), never assuming root repository paths.
